  ...
  <div>
    ...
    <header>
      <div class="container">
        <div class="row py-3">
          <div class="col">
            <h1 class="mb-1">{{ $t("main.menu.working") }}</h1>
          </div>
            ...
            <div class="modal-content">
              ...
              <div class="modal-footer border-0">
                ...
                <button
                  ...
                  @click="updateClaimStatus(Global.claimObjSelect.status)"
                >
                   {{ $t( "mainmenu.label.resume") }}
                </button>
              </div>
...
export default {
  ...
  methods: {
        ...
        vm.axios
          ...
          .then(function(response) {
            ...
            if (response && response.data.code && response.data.code == "OK") {
              ...
              // vm.claimAr[index].policyDetail = JSON.parse(response.data.data);
              vm.$set(
                vm.claimAr[index],
                "policyDetail",
                response.data.data
          ...
          .catch(error => {
            $("#processing").modal("hide");
            console.log("getPolicyBySfid Exception:", error);
          });
      }
    },
    openMapinGoogle: function(claim) {
      var vm = this;
      vm.showClaimCase(claim);
      var URL =
    ...
    validateFormConf: function(status) {
      ...
      if (msgValidate == "") {
        ...
        vm.$swal("Please Enter Data!!!", msgValidate, "error");
        return false;
      }
    },
    goToWorkActive: function(claim) {
      var vm = this;
      vm.$set(Global, "claimObjSelect", claim);
      vm.$set(Global, "claimObject", claim);
      vm.$router.push("/workactive");
    },
    updateClaimStatus: function(claim) {
      var vm = this;
      $("#processing").modal("show");
      vm.showClaimCase(claim);
      var status = Global.claimObjSelect.status;
      ...
      if (vm.validateFormConf(status)) {
        ...
        if (status == "Accepted") {
          ...
          formData.append("survEndLat", Global.surveyorObj.sLastLat);
          formData.append("survEndLng", Global.surveyorObj.sLastLng);
          formData.append("survEndAddress", Global.currentAdress);
          formData.append("fcmToken", localStorage.getItem("fcm_token"));
          // formData.append("fcmToken", "FAKEFCMTOKEN");
          formData.append("surveyorId", user_obj.id);
        }
          ...
          .then(function(response) {
            ...
            if (
              ...
              if (status == "Accepted" || status == "Arrived") {
                // vm.$centerfx.displayMdInform('#md_accept_case', 'hide');
              ...
              console.log("Update response fail");
    ...
    getPolicyBySfid: function(claim) {
      var vm = this;
      $("#processing").modal("show");
      if (claim.policyObj) {
      } else {
        ...
        this.axios
          ...
          .then(function(response) {
            ...
            if (response && response.data.code && response.data.code == "OK") {
              ...
              claim.policyObj = response.data.data;
              // $("#md_policy").modal("show");
              // vm.policyObj =[{modelFamilyC:"Vios", yearOfManufactureC:"2016", contractname:"1000-00914642", makeC:"Toyota", chassisNumberC:null, carPlateNumberC:"4e3ert", endDateC:"2020-04-08", carPlateProvinceC:null, status:"Activated"}]
            }
          })
          .catch(error => {
    ...
    getClaimNoRealtime: function() {
      ...
      formData.append("status", "'Accepted','Arrived'");
      formData.append("surveyorId", userObj.surveyorAccountC);
      // formData.append("encryptedCustomerId",localStorage.getItem("encryptedCustomerId"));
      ...
      this.axios
        ...
        .catch(error => {
          $("#processing").modal("hide");
          console.log("!!ERROR getClaimNoRealtime", error);
        });
    },
    getClaimByCondition: function(flagNotShow) {
      ...
      this.axios
        ...
        .catch(error => {
          $("processing").modal("hiide");
          console.log("!!ERROR getClaimByConditions", error);
        });
    },
  ...
  mounted() {
    ...
    if (localStorage.getItem("case_obj")) {
      vm.flagBusyWork = true;
    } else {
      vm.flagBusyWork = false;
    }
    vm.setUnlockScreen();
  },
  beforeDestroy() {
    console.log("beforeDestroy!!! Working");
    var vm = this;
